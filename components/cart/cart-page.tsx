...
export function CartPage({ data }: CartPageProps) {
  ...
  const { cart, totals, removeItem, updateQuantity, clearCart } = useCart();
      ...
      <div className="app-shell py-5">
        <header className="mb-6 flex items-center justify-between gap-3">
          <div>
            ...
            <p className="text-sm font-semibold text-muted">{t("table")} {cart.tableCode}</p>
          </div>
          <LanguageSelector />
        </header>
                ...
                <article key={item.lineId} className="grid grid-cols-[92px_1fr] gap-3 rounded-[var(--radius-brand)] border border-border bg-surface p-3 shadow-sm">
                  ...
                  <div className="min-w-0 space-y-2">
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <p className="shrink-0 text-sm font-black">{formatMoney(cartLineTotal(item))}</p>
                    </div>
            ...
            <aside className="h-max rounded-[var(--radius-brand)] border border-border bg-surface p-5 shadow-sm">
              ...
              <dl className="mt-4 space-y-3 text-sm font-semibold">
                ...
                <div className="flex justify-between gap-4">
                  ...
                  <dd>{formatMoney(totals.serviceCharge)}</dd>
                </div>
                <div className="flex justify-between gap-4">
              ...
              <div className="mt-4 grid gap-2">
                <Link href={`${basePath}/order-status`} className="touch-target inline-flex items-center justify-center rounded-full bg-primary px-4 text-sm font-extrabold text-white">
                  {t("orderStatus")}
                </Link>
